...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil {
    if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])) {
        self.title = LocalizedString(@"Preferences");
...
{
    switch (section) {
        case 0: return LocalizedString(@"Web Interface");
        case 1: return LocalizedString(@"Network Interface");
        case 2: return LocalizedString(@"Port Listening");
        case 3: return LocalizedString(@"Logging");
        case 4: return LocalizedString(@"Background Downloading");
    }
    return nil;
}
...
{
    switch (section) {
        case 0: return [NSString stringWithFormat:LocalizedString(@"It's always recommended to use authentication if web interface is enabled. \n\nYour IP address is: %@ \n\nRemember the file server's port number is always one port above the web interface."), [UIDevice getAbsoluteLocalIPAddress]];
        case 1: return LocalizedString(@"Enabling cellular network may generate significant data charges.");
        case 2: return nil;
        case 3: return LocalizedString(@"Only use logging for debugging. Extensive loggings will shorten both battery and Nand life. Saved logs will be available in iTunes.");
        case 4: return LocalizedString(@"Enable downloading while in background.");
    }
    return nil;
}
...
- (void)portCheckerDidFinishProbing:(PortChecker*)c
{
	...
	if ([c status] == PORT_STATUS_OPEN) {
		msg = [NSString stringWithFormat:LocalizedString(@"Congratulations. Your port %li is open!"), (long)[c portToCheck]];
	}
	if ([c status] == PORT_STATUS_ERROR) {
		msg = LocalizedString(@"Failed to perform port check.");
	}
	if ([c status] == PORT_STATUS_CLOSED) {
		msg = [NSString stringWithFormat:LocalizedString(@"Oh bad. Your port %li is not accessable from outside."), (long)[c portToCheck]];
	}
...
- (void)saveButtonClicked
{
    ...
    if ([fEnableRPCSwitch isOn] != [self.originalPreferences boolForKey:@"RPC"]) {
        ...
        if ([fEnableRPCSwitch isOn]) {
            [[WebHelperFileService sharedInstance] startServiceOnPort:(rpc_port + 1)];
        } else {
    ...
    if (![[fRPCPasswordTextField text] isEqualToString:[self.originalPreferences stringForKey:@"RPCPassword"]]) {
        [fDefaults setObject:[fRPCPasswordTextField text] forKey:@"RPCPassword"];
        tr_sessionSetRPCPassword(fHandle, [[fRPCPasswordTextField text] cStringUsingEncoding:NSUTF8StringEncoding]);
    }
	...
	if (callSetNetworkActive)
		[self.controller updateNetworkStatus];
...
- (void)viewDidLoad {
    ...
    [[WebHelperFileService sharedInstance] setDelegate:self];
