...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
	if ((self = [super initWithNibName:nibNameOrNil bundle:nibBundleOrNil])) {
        self.title = LocalizedString(@"Global Bandwidth");
...
- (void)setTorrent:(Torrent *)torrent
{
    ...
    if (torrent) {
        ...
        self.title = LocalizedString(@"Bandwidth");
    }
}
...
- (NSString*)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
    switch (section) {
        case 0:
            return LocalizedString(@"Connections");
            break;
        case 1:
            if (self.torrent) return LocalizedString(@"Speed Limits");
            else return LocalizedString(@"Upload");
        case 2:
            return LocalizedString(@"Download");
        default:
            break;
    }
    return nil;
...
{
    if (section == 0)
        return LocalizedString(@"Caution! Too many connections will make your device unstable.");
    else if (section == 1)
        return LocalizedString(@"30KB/s is recommended for upload.");
    else
        return nil;
}
